...
const MAX_RETRIES = 2        // 1 retry max (2 total attempts)
const RETRY_DELAY_MS = 5000  // 5s, 10s backoff
export const AGENT_TIMEOUT_MS = 90_000   // default: 90s hard timeout per call
export const SEARCH_AGENT_TIMEOUT_MS = 180_000 // quality-checker + revision (web search / full rewrite)
...
function withTimeout(promise, timeoutMs = AGENT_TIMEOUT_MS) {
  return Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`Agent call timed out after ${timeoutMs / 1000}s`)), timeoutMs)
    ),
  ])
}
 ...
 */
export async function ask(systemPrompt, userPrompt, maxTokens = 4096, timeoutMs = AGENT_TIMEOUT_MS) {
  let lastError
 ...
 */
export async function askWithSearch(systemPrompt, userPrompt, maxTokens = 4096, timeoutMs = AGENT_TIMEOUT_MS) {
  let lastError
