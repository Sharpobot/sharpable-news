...
- A well-written, factually grounded Bahasa Malaysia article following the style guide with verified facts, proper structure, and no critical issues SHOULD score at least 85/100 by default. Be generous for solid articles.
- "publish" verdict requires overallScore >= 85
- "review" verdict requires overallScore between 60 and 84
...
- If you give a "review" verdict, overallScore must be 60-84. If "publish", must be 85-100.
...
Return a JSON object in this exact format:
{
  ...
  "publishReadiness": "ready | needs_minor_fixes | needs_major_revision"
    ...
    2500   // raised from 1200 — web search synthesis + JSON can exceed 1200 and truncate
  )

  const verdict = result.verdict ?? 'reject'
  const passed = verdict === 'publish'

  // Guard against LLM returning 0/null, OR JSON being truncated (result.raw exists)
  // result.raw means parseJSON failed — treat as reject with low-but-not-zero score
  const rawScore = result.overallScore ?? 0
  const overallScore = rawScore > 0
    ? rawScore
    : verdict === 'publish' ? 87   // above new 85 publish threshold
    : verdict === 'review'  ? 70   // mid range for review (60–84)
    : 35   // reject floor — 0 means parsing failed, not that everything is broken
