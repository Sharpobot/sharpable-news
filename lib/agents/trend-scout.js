...
Return a JSON object in this exact format:
{
  ...
  console.log('[trend-scout] raw result keys:', Object.keys(result))
  console.log('[trend-scout] trends array length:', result.trends?.length ?? 'undefined')
  console.log('[trend-scout] trends:', JSON.stringify(result.trends ?? [], null, 2).slice(0, 1000))
  if (!result.trends || result.trends.length === 0) {
    console.warn('[trend-scout] ⚠️ No trends returned. Full result:', JSON.stringify(result).slice(0, 800))
  }
  // ── END DEBUG ───────────────────────────────────────────
