...
export async function trendScout(context = {}, topicDirection = null) {
  ...
  const directionBlock = topicDirection
    ? `\n\nTOPIC DIRECTION: "${topicDirection}"

MANDATORY SEARCH PROTOCOL — follow in order:
1. Your FIRST web search MUST use the exact keywords from the topic direction above as the primary query. Search specifically for "${topicDirection}" news from the last 48 hours.
2. Evaluate results: if you find 3 or more distinct, newsworthy stories directly related to "${topicDirection}", focus your trend list on those results.
3. Only if you find FEWER THAN 3 relevant results for "${topicDirection}" should you broaden your search to general AI/tech trending topics.
4. In your final JSON output, place topics that directly match "${topicDirection}" FIRST in the trends array.`
    : ''
...
Always respond with valid JSON only — no markdown, no extra text.`,
    ...
    `Today's date is ${today}.${directionBlock}
