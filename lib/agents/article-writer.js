...
export async function articleWriter(context = {}) {
  ...
  // Target word count — default standard (750), overridden by site_settings if present
  const WORD_COUNT_BY_LENGTH = {
    brief: {
      instruction: 'kira-kira 600 patah perkataan. Fokus pada fakta teras sahaja — ayat padat dan ringkas, tanpa huraian atau ayat tambahan yang tidak perlu',
      target: 600,
    },
    standard: {
      instruction: 'kira-kira 750 patah perkataan. Liputan seimbang dengan konteks dan sedikit analisis — tahap lalai',
      target: 750,
    },
    detailed: {
      instruction: 'kira-kira 900 patah perkataan. Liputan lebih mendalam yang meneroka implikasi dan latar belakang — tegas, tiada ayat pengisi (filler); setiap ayat mesti membawa maklumat baharu',
      target: 900,
    },
  }
  let wordCountSetting = WORD_COUNT_BY_LENGTH.standard
  ...
  try {
    ...
    const { data: settings } = await db
      .from('site_settings')
      .select('value')
      .eq('key', 'target_article_length')
      .maybeSingle()

    if (settings?.value && WORD_COUNT_BY_LENGTH[settings.value]) {
      wordCountSetting = WORD_COUNT_BY_LENGTH[settings.value]
    }
  } catch {
...
Tulis artikel berita penuh dalam Bahasa Malaysia (${wordCountSetting.instruction}).
Panduan gaya di atas SENTIASA mengatasi sasaran panjang ini — nada, gaya, tahap bahasa, dan format tajuk mesti kekal sama tidak kira panjang artikel. Panjang hanya mengubah kedalaman liputan dan bilangan perenggan, bukan daftar bahasa atau struktur ayat.
...
Kembalikan objek JSON dalam format TEPAT ini:
{
  ...
  "wordCount": ${wordCountSetting.target},
