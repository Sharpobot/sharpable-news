...
import { PIPELINE_MESSAGES } from './pipeline-messages.js'
...
/** Lowercase slug: spaces → hyphens, strip non-alphanumeric (keep hyphens). */
...
/** First 160 chars of plain text from a TipTap JSON body. */
function extractBodyPlainText(body, maxLen = 160) {
...
export const generateArticle = inngest.createFunction(
  ...
  async ({ event, step, attempt }) => {
    ...
    const ctx1 = await step.run('trend-scout', async () => {
      const pid = await startProgress('trend-scout', 'Searching for trending topics…')
      try {
        const ctx = await trendScout({}, topicDirection ?? null)
        await endProgress(pid, 'done', `Found ${ctx.trends?.length ?? 0} trending topics`)
        return ctx
    ...
    const ctx2 = await step.run('topic-selector', async () => {
      const pid = await startProgress('topic-selector', PIPELINE_MESSAGES.TOPIC_SELECTOR.start)
      try {
        const ctx = await topicSelector({ ...ctx1, existingArticles })
        await endProgress(pid, 'done', PIPELINE_MESSAGES.TOPIC_SELECTOR.done(ctx.topicOptions?.length ?? 0))
        return ctx
      } catch (err) { await endProgress(pid, 'failed', err.message); checkFailureLimit(err) }
    })
    ...
    if (!ctx2.topicOptions || ctx2.topicOptions.length === 0) {
      ...
      await db.from('articles').update({
        status: 'failed',
        quality_flags: { verdict: 'reject', error: 'No trending topics found — try again shortly.' },
      }).eq('id', articleId)
      throw new Error('No trending topics found — try again shortly.')
    }
    ...
    await step.run('save-topic-options', async () => {
      ...
      const { error } = await db
        ...
        .update({
          ...
          topic_direction: topicDirection ?? null,
        })
        .eq('id', articleId)
      ...
      await db.from('article_generation_progress').insert({
        ...
        status: 'running',
        message: 'Waiting for editor to select a topic…',
      })
    })
    ...
    if (!selection) {
      ...
      await db.from('articles').update({
        status: 'failed',
        quality_flags: { verdict: 'reject', error: 'Topic selection timed out — try generating again.' },
      }).eq('id', articleId)
    ...
    await step.run('resume-pipeline', async () => {
      ...
      await db.from('article_generation_progress')
        .update({ status: 'done', message: `Topic selected: ${selectedOption?.topic ?? ''}` })
    ...
    const ctx3 = await step.run('deep-researcher', async () => {
      const pid = await startProgress('deep-researcher', 'Running deep research...')
      try {
        const ctx = await deepResearcher(selectedTopicCtx)
        const factCount = ctx.researchBrief?.keyFacts?.length ?? 0
        await endProgress(pid, 'done', `Brief ready — ${factCount} key facts gathered`)
        return ctx
      } catch (err) {
    ...
    const ctx4 = await step.run('article-writer', async () => {
      const pid = await startProgress('article-writer', 'Writing article in Bahasa Malaysia...')
      try {
        const ctx = await articleWriter(ctx3)
        const words = ctx.article?.wordCount ?? 0
        await endProgress(pid, 'done', `Article ready — ${words} words`)
        return ctx
      } catch (err) {
        await endProgress(pid, 'failed', err.message)
        throw err
    ...
    const ctx5 = await step.run('seo-metadata', async () => {
      const pid = await startProgress('seo-metadata', 'Generating SEO metadata...')
      try {
        const ctx = await seoMetadata(ctx4)
        await endProgress(pid, 'done', `Slug: ${ctx.seo?.slug ?? '(none)'}`)
        return ctx
      } catch (err) {
        await endProgress(pid, 'failed', err.message)
        throw err
    ...
    const ctx6 = await step.run('image-brief', async () => {
      const pid = await startProgress('image-brief', 'Preparing image brief...')
      try {
        const ctx = await imageBrief(ctx5)
        await endProgress(pid, 'done', 'Hero and inline image briefs ready')
        return ctx
      } catch (err) {
        await endProgress(pid, 'failed', err.message)
        throw err
    ...
    const ctx7 = await step.run('quality-checker', async () => {
      const pid = await startProgress('quality-checker', 'Checking quality and facts...')
      try {
        ...
        await endProgress(pid, 'done', `Score: ${score}/100 — ${verdict}`)
        return ctx
      } catch (err) {
        await endProgress(pid, 'failed', err.message)
        throw err
    ...
    if (needsRevision(ctx7.qualityReport)) {
      ...
      const ctx8 = await step.run('revision-agent', async () => {
        const pid = await startProgress('revision-agent', 'Revising article based on quality review...')
        try {
          const ctx = await revisionAgent(ctx7)
          const fixes = ctx.revisionCorrectionsMade?.length ?? 0
          await endProgress(pid, 'done', `${fixes} corrections made`)
          return ctx
        } catch (err) {
          await endProgress(pid, 'failed', err.message)
          throw err
      ...
      const ctx9 = await step.run('quality-checker-2', async () => {
        const pid = await startProgress('quality-checker', 'Quality check #2 after revisions...')
        try {
          const ctx = await qualityChecker(ctx8)
          const score   = ctx.qualityReport?.overallScore ?? 0
          const verdict = ctx.qualityReport?.verdict ?? 'unknown'
          await endProgress(pid, 'done', `Check #2 score: ${score}/100 — ${verdict}`)
          return ctx
        } catch (err) {
          await endProgress(pid, 'failed', err.message)
          throw err
      ...
      if (needsRevision(ctx9.qualityReport)) {
        ...
        const ctx10 = await step.run('revision-agent-2', async () => {
          const pid = await startProgress('revision-agent', 'Second revision pass to reach 85+...')
          try {
            ...
            const fixes = ctx.revisionCorrectionsMade?.length ?? 0
            await endProgress(pid, 'done', `Second pass complete — ${fixes} corrections`)
            return ctx
          } catch (err) {
            await endProgress(pid, 'failed', err.message)
            throw err
    ...
    await step.run('save-article', async () => {
      const savePid = await startProgress('save-article', 'Saving article to database...')
      try {
      ...
      const title = ctxFinal.article?.headlines?.[0] ?? null
      const slug =
        (ctxFinal.seo?.slug && String(ctxFinal.seo.slug).trim()) ||
        slugify(title) ||
        `article-${articleId.slice(0, 8)}`
      const metaDescription =
        (ctxFinal.seo?.metaDescription && String(ctxFinal.seo.metaDescription).trim()) ||
        extractBodyPlainText(ctxFinal.article?.body) ||
        null

      console.log('[save-article] context before Supabase insert:', JSON.stringify({
        articleId,
        title,
        slug,
        metaDescription,
        seo: ctxFinal.seo ?? null,
        headlineCount: ctxFinal.article?.headlines?.length ?? 0,
        hasBody: !!ctxFinal.article?.body,
        tagCount: ctxFinal.seo?.tags?.length ?? 0,
        qualityScore: ctxFinal.qualityReport?.overallScore ?? null,
      ...
      await endProgress(savePid, 'done', 'Article ready for review')
      ...
      if (attempt >= 1) {
        try {
          ...
          await failDb.from('articles').update({
            ...
            quality_flags: {
              verdict: 'reject',
              error: err.message ?? 'Pipeline failed — try generating again.',
            },
          }).eq('id', articleId)
          ...
          if (pending.length > 0) {
            await failDb.from('article_generation_progress').insert(
              pending.map(agent_name => ({
                ...
                agent_name,
                status: 'failed',
                message: 'Pipeline failed before this step ran',
              }))
            )
          }
        } catch (markErr) {
