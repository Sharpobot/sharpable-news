...
export const generateArticle = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // ── Step 7: Quality Checker (first pass) ───────────────────
    ...
    // Only runs if quality verdict is not 'publish'
    let ctxFinal = ctx7
    ...
    if (ctx7.qualityReport?.verdict !== 'publish') {
      ...
      await step.sleep('wait-before-recheck', '65s')

      // ── Step 9: Quality Checker (second pass after revision) ──
      const ctx9 = await step.run('quality-recheck', async () => {
        const pid = await startProgress('quality-checker', 'Semakan kualiti semula selepas pembetulan...')
        try {
          const ctx = await qualityChecker(ctx8)
          const score = ctx.qualityReport?.overallScore ?? 0
          const verdict = ctx.qualityReport?.verdict ?? 'unknown'
          await endProgress(pid, 'done', `Skor akhir: ${score}/100 — ${verdict}`)
          return {
            ...ctx,
            // Preserve original quality flags from first pass
            originalQualityReport: ctx7.qualityReport,
            revisionCorrectionsMade: ctx8.revisionCorrectionsMade,
          }
        } catch (err) {
          await endProgress(pid, 'failed', err.message)
          throw err
    ...
    await step.run('save-article', async () => {
      ...
      const update = {
        ...
        image_brief: ctxFinal.images?.heroImage
          ? `${ctxFinal.images.heroImage.description} | Query: ${ctxFinal.images.heroImage.unsplashQuery}`
          : null,
        ...
        // Original quality flags (before revision) — null if no revision ran
        original_quality_flags: ctxFinal.originalQualityReport
          ? {
