...
export const generateArticle = inngest.createFunction(
  ...
  async ({ event, step, attempt }) => {
    ...
    await step.run('save-topic-options', async () => {
      ...
      // Phase 1: update status (always required)
      const { error: statusErr } = await db.from('articles').update({
        status: 'awaiting_topic_selection',
      }).eq('id', articleId)
      if (statusErr) console.error('[save-topic-options] status update error:', statusErr.message)

      // Phase 2: save topic_options column (requires migration 007 — non-fatal if missing)
      const { error: optErr } = await db.from('articles').update({
        topic_options: topicOptionsCtx.topicOptions ?? [],
      }).eq('id', articleId)
      if (optErr) console.warn('[save-topic-options] topic_options column missing — apply migration 007:', optErr.message)

      // Phase 3: write waiting progress row
