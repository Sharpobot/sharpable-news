  ...
  async ({ event, step }) => {
    ...
    await step.run('save-article', async () => {
      ...
      const update = {
        ...
        // Image brief — use .prompt (AI generation prompt) with Unsplash fallback query
        ...
        // Similar articles audit trail (from topic deduplication)
        similar_articles: ctx2.similarArticles?.length > 0
          ? ctx2.similarArticles
          : null,
