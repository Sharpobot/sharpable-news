...
export const generateArticle = inngest.createFunction(
  ...
  async ({ event, step, attempt }) => {
    ...
    // ── Steps 1+2: Trend Scout → Topic Selector (with dedup retry) ──
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      const ctx2Candidate = await step.run(`topic-selector${suffix}`, async () => {
        const pid = await startProgress('topic-selector', attempt === 0
      ...
      if (!ctx2Candidate.isDuplicate) { ctx2 = ctx2Candidate; break }
